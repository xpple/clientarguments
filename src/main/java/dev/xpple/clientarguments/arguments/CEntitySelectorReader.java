...
public class CEntitySelectorReader {
	public static final SimpleCommandExceptionType INVALID_ENTITY_EXCEPTION = new SimpleCommandExceptionType(new TranslatableText("cargument.entity.invalid"));
	public static final DynamicCommandExceptionType UNKNOWN_SELECTOR_EXCEPTION = new DynamicCommandExceptionType(selectorType -> new TranslatableText("cargument.entity.selector.unknown", selectorType));
	...
	public static final SimpleCommandExceptionType MISSING_EXCEPTION = new SimpleCommandExceptionType(new TranslatableText("cargument.entity.selector.missing"));
	public static final SimpleCommandExceptionType UNTERMINATED_EXCEPTION = new SimpleCommandExceptionType(new TranslatableText("cargument.entity.options.unterminated"));
	public static final DynamicCommandExceptionType VALUELESS_EXCEPTION = new DynamicCommandExceptionType(option -> new TranslatableText("cargument.entity.options.valueless", option));
	...
	public CEntitySelector build() {
		...
		if (this.dx == null && this.dy == null && this.dz == null) {
			if (this.distance.getMax() != null) {
				...
				box3 = new Box(-d, -d, -d, d + 1.0D, d + 1.0D, d + 1.0D);
			} else {
				box3 = null;
			}
		} else {
		...
		if (this.x == null && this.y == null && this.z == null) {
			...
			function2 = (pos) -> new Vec3d(this.x == null ? pos.x : this.x, this.y == null ? pos.y : this.y, this.z == null ? pos.z : this.z);
		}

		return new CEntitySelector(this.limit, this.includesNonPlayers, this.predicate, this.distance, function2, box3, this.sorter, this.senderOnly, this.playerName, this.uuid, this.entityType, this.usesAt);
	}
	...
	private void buildPredicate() {
		...
		if (!this.levelRange.isDummy()) {
			this.predicate = this.predicate.and(entity -> entity instanceof AbstractClientPlayerEntity && this.levelRange.test(((AbstractClientPlayerEntity) entity).experienceLevel));
		}
	}
	...
	protected void readArguments() throws CommandSyntaxException {
		...
		while (this.reader.canRead() && this.reader.peek() != ']') {
			...
			int cursor = this.reader.getCursor();
			String string = this.reader.readString();
			CEntitySelectorOptions.SelectorHandler selectorHandler = CEntitySelectorOptions.getHandler(this, string, cursor);
			this.reader.skipWhitespace();
			if (this.reader.canRead() && this.reader.peek() == '=') {
				this.reader.skip();
				this.reader.skipWhitespace();
	...
	private static void suggestSelector(SuggestionsBuilder builder) {
		builder.suggest("@p", new TranslatableText("cargument.entity.selector.nearestPlayer"));
		builder.suggest("@a", new TranslatableText("cargument.entity.selector.allPlayers"));
		builder.suggest("@r", new TranslatableText("cargument.entity.selector.randomPlayer"));
		builder.suggest("@s", new TranslatableText("cargument.entity.selector.self"));
		builder.suggest("@e", new TranslatableText("cargument.entity.selector.allEntities"));
	}
